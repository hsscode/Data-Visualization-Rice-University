...
def filter_by_year(statistics, year, yearid):
    ...
    for stat in statistics:
        if stat[yearid] == str(year):  # Ensure year comparison is in string format
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    player_stats = [(stat[info["playerid"]], formula(info, stat)) for stat in statistics]
    sorted_player_stats = sorted(player_stats, key=lambda x: x[1], reverse=True)
    return sorted_player_stats[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    player_names_and_stats = []
    # Load player data from the CSV file into a dictionary
    player_data = read_csv_as_nested_dict(info['masterfile'], info['playerid'], info['separator'], info['quote'])
    
    for player_id, stat in top_ids_and_stats:
        # Retrieve first name and last name from the player data dictionary
        player_info = player_data.get(player_id, {})
        first_name = player_info.get(info['firstname'], "")
        last_name = player_info.get(info['lastname'], "")
        player_names_and_stats.append(f"{stat:.3f} --- {first_name} {last_name}")
    
    return player_names_and_stats
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    # Step 1: Filter batting statistics data for the specified year
    ...
    # Step 2: Compute compound statistic for each player
    player_stats = [(player[info['playerid']], formula(info, player)) for player in year_stats]
    
    # Step 3: Sort players based on computed statistic in descending order
    sorted_players = sorted(player_stats, key=lambda x: x[1], reverse=True)
    
    # Step 4: Take top numplayers players from the sorted list
    top_players = sorted_players[:numplayers]
    
    # Step 5: Format output as list of strings
    top_player_names_and_stats = lookup_player_names(info, top_players)
    
    return top_player_names_and_stats
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for player_stats in statistics:
        ...
        if player_id not in aggregated_stats:
            ...
            for field in fields:
                aggregated_stats[player_id][field] = 0
        for field in fields:
            if field in player_stats:
                aggregated_stats[player_id][field] += int(player_stats[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    Inputs:
      ...
      numplayers  - Number of top players to return
    Outputs:
      Returns a list of strings for the top numplayers players according to the given formula.
    """
    ...
    top_players = sorted(aggregated_stats.items(), key=lambda x: formula(info, x[1]), reverse=True)[:numplayers]
    player_names = []
    for player_id, _ in top_players:
        player_info = read_csv_as_nested_dict(info['masterfile'], info['playerid'], info['separator'], info['quote'])
        first_name = player_info[player_id][info['firstname']]
        last_name = player_info[player_id][info['lastname']]
        stat = formula(info, aggregated_stats[player_id])
        player_names.append(f"{stat:.3f} --- {first_name} {last_name}")
    return player_names
